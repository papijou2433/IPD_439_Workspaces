...
def capture_uart_data():
    ...
    try:
        with serial.Serial(PORT, BAUDRATE, timeout=TIMEOUT) as ser:
            # --- Configuración vital para el USB Nativo CDC del ESP32-S3 ---
            ser.dtr = True
            ser.rts = True
            print("Esperando 2 segundos a que el USB CDC se estabilice...")
            time.sleep(2) 
            
            # Limpiar cualquier residuo en el buffer antes de empezar
            ser.reset_input_buffer() 
            # ---------------------------------------------------------------

            # Enviar byte 0x01 para iniciar la conversión ADC y el Timer
            print("Enviando comando de inicio (0x01)...")
            ser.write(b'\x01')
            ser.flush() # Forzar salida inmediata del comando
            ...
            while lines_received < SAMPLES:
                ...
                try:
                    ...
                    # Ignorar las tramas de temperatura provenientes de ADC1/TIM3 si existieran
