...
%\thanks{Este trabajo está licenciado bajo una licencia \href{https://creativecommons.org/licenses/by-nc/4.0/}{Creative Commons Attribution-NonCommercial 4.0 International}  \includegraphics[height=1.5em]{by-nc.png}.}}
\date{Abril 2026}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.4\linewidth]{figures/diagrama_SAR.png}
    \caption{Arquitectura general de un SAR ADC}
    \label{fig:sar}
\end{figure}
...
\begin{itemize}
    ...
    \end{equation}
    Sin embargo, debido a que los datos de la medición son almacenados en forma de histograma, más que señal continua del tiempo. El método de medición es el siguiente:
    \begin{equation}
    	DNL(k) = \frac{h(k)_{real}}{h(k)_{esperado}}-1
    \end{equation}
    Un valor de $DNL(k) = -1$ LSB indicaría la presencia de códigos perdidos en ese código k.
...
\begin{description}
    ...
    \item[OE3:] Caracterizar el rendimiento de los ADC de estos microcontroladores ante distintas temperaturas
\end{description}
\section{Metodología}
...
\begin{figure}[h]
    ...
    \label{fig:entorno}
\end{figure}
El firmware implementado para obtener las características del DAC en la STM32 mantiene una arquitectura de ejecución simple. Iniciando el proceso de captura mediante la recepción de un comando de 8 bits mediante UART, dando inicio al proceso de conversión y activación del timer 2. Transcurrido el tiempo de adquisición determinado por la cantidad de datos a medir, se detiene el timer y se da inicio al procesamiento de datos mediante el archivo .csv generado por el programa waveforms.
\\
De manera similar, el firmware implementado para obtener las características del ADC de ambos microcontroladores mantienen semejanza con lo realizado para el DAC. En este caso, se añade la medición de temperaturas al inicio y al final de la conversión. Es importante destacar que, las mediciones de temperaturas son realizadas mediante el sensor interno de temperatura y posteriormente procesadas mediante un ADC, este siendo el ADC1 en caso de la STM32 y un ADC dedicado en arquitectura delta sigma en caso de la ESP32.
\\
Finalmente, se destaca que el firmware implementado para obtener las métricas del ADC es el mismo para obtener métricas DC y AC, en ambos casos la frecuencia de toma de muestras es de 10kHz. Esto debido a que no se logró habilitar el uso de DMA directamente con el ADC de la tarjeta ESP32 para poder obtener una frecuencia de toma de muestras de 100kHz, de todas formas, se encuentra disponible en el repositorio de este proyecto el uso de DMA de forma continua para el ADC de la STM32 a una frecuencia de 100kHz.
\section{Resultados}
